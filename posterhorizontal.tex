...
\author{Maria Coto- Sarmiento$^{1,2}$, Simon Carrignon$^{1}$ and Xavier Rubio-Campillo$^{1}$} % Author(s)

\institute{$^1$Barcelona Supercomputing Center - $^2$University of Barcelona} % Institution(s)
...
{\textbf{Model }}\\
\justify
We propose a model based on classical random drift model to test the impact of horizontal vs vertical transmission (HT vs VT) on the variation in production between workshops.  %and to give an idea on which conditions can explain the variation revealed by the PCA. 
