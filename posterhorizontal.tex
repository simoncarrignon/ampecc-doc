...
\usepackage[scale=.8,papersize=a1]{beamerposter} % Use the beamerposter package for laying out the poster
...
\begin{figure}
	...
	\draw (\x cm,2pt) -- (\x cm,-2pt) node[anchor=north] {$WS_\x$};
		...
		\draw [black,shorten <= 0.25cm, shorten >= 0.25cm, <->] (WS1) to[out=80,in=100,distance=1cm] node[above,font=\scriptsize]{knowledge transmission} (WS4); 
