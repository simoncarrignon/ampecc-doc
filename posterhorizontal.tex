...
PCA was used to explore these metrical observations with the 8 measurement as variables\xavi{This sentence is redundant (observations with measurement as variables). Better phrasing: Principal Component Analysis (PCA) allowed us to capture most of the variance of the 8 measurements into 2 variables}. \xavi{You should use an acronym only if you use it a lot. If you just use it 2 times it's better to use the full term}
...
{\textbf{Model }}
\justify
The observed pattern was further explored with a model based on classical random drift~\cite{bentley2004randomdriftandculturechange}. This approach can be used to to test the impact of horizontal vs vertical transmission (HT vs VT) on the variation in production between workshops.  %and to give an idea on which conditions can explain the variation revealed by the PCA. 

A set $Pop$ of $N$ workshops sharing the same initial production techniques $P^{0}$ are positioned along a line at increasing distances\xavi{Why do you explain just the initialization of the algorithm? Either you explain all of it, or explain the general process and then you show the algorithm.}. This simplification of spatial distance is based on the fact that all workshops are positioned along the same river course.% (Fig~\ref{fig:mod},~left). %In this model workshops produce a certain number of amphorae \& can: (1) change their production technics following a random inovation procedure (vertical transmission) or (2) copy the production techniques of another workshop.
...
\xavi{what if you put the paragraph above before the algorithm?}
...
Empirical studies has identified variation on the making techniques processes among pottery workshops. We observe that this variability is affected by the distance: the analysed morphometric traits show that the similarity between amphorae decrease with the spatial distance between the workhops they were produced. It suggests that the intensity of horizontal transmission was low and the process was guided mainly by vertical transmission. A possible interpretation is that ottery techniques were learned from master to disciple, and the disciples remained in the workhop where they were trained.


The combination of this empirical analysis with the theoretical model suggests that horizontal transmission could not be the main cultural process in the workshops. This result, combined with the correlation between spatial distance and morphometric distance suggests that vertical transmission is probably the most probable mechanism\xavi{This sentence is repeating the information of the previous paragraph}.
